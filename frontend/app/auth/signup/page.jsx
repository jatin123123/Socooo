...
function SignupPage() {
  ...
  const handelSubmit=async(e)=>{
    e.preventDefault();
    try {
      const response=await axios.post(`${baseurl}/api/signup`,{username,UserImage,email,password},{withCredentials:true})
  ...
  return (
    <div className='w-full flex items-center justify-center h-screen'>
        ...
        {/* Form Starts */}
        <form className='mt-7 flex flex-col  md:flex-row  md:gap-28  items-center gap-4' onSubmit={handelSubmit}>
       ...
       <div className=' md:w-[80%] '>
        ...
        <p className='  text-[12px] text-zinc-400 mr-7 tracking-tighter font-extralight '>Already have an account:<Link  href={'/auth/login'} className="text-black opacity-100 hover:border-b border-black transition-all ">Sign-In</Link> </p>
        </div>
        </form>
            </div>
    </div>
